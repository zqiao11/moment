...
from tscil.model.prompt import Prompt
...
class L2pMOMENT(nn.Module):
    def __init__(self, config: Namespace | dict, **kwargs: dict):
        ...
        self.patch_embedding = PatchEmbedding(
            ...
            stride=config.patch_stride_len,
            dropout=config.getattr("dropout", 0.1),
            add_positional_embedding=config.getattr("add_positional_embedding", False),  # ZZ: Set to False, add pe after prompting
            value_embedding_bias=config.getattr("value_embedding_bias", False),
            orth_gain=config.getattr("orth_gain", 1.41),
        )
        self.position_embedding = PositionalEmbedding(d_model=config.d_model, model_name="L2pMOMENT")
        ...
        # Prompt related
        # ToDo: Do we need cls_token? Moment doesn't use it for classification.
        #  But in fact, it can be introduced. We just keep not using it as Moment atm.
        # self.cls_token = nn.Parameter(torch.zeros(1, 1, config.d_model)) if class_token else None

        # embed_len = num_patches if no_embed_class else num_patches + self.num_prefix_tokens
        # if config.prompt_length is not None and config.pool_size is not None and config.prompt_pool:
        #     embed_len += config.prompt_length * config.top_k

        self.prompt_pool = config.prompt_pool  # False
        self.use_prompt_mask = config.use_prompt_mask  # False
        self.prompt = Prompt(
            length=config.prompt_length,
            embed_dim=config.d_model,
            embedding_key=config.embedding_key,
            prompt_init=config.prompt_key_init,
            prompt_pool=config.prompt_pool,
    ...
    def classify(
        ...
        **kwargs,
        ...
        # [batch_size x n_channels, n_patches, d_model]
        # ToDo: Add prefix prompt after flattening
        #  May consider to add prompt for each channel

        # ZZ: For now, we don't use prompt_mask, as SplitCifar100
        ...
        prompt_mask = None
        cls_features = None
        res = self.prompt(enc_in, prompt_mask=prompt_mask, cls_features=cls_features)
        enc_in = res['prompted_embedding']
        reduce_sim = res['reduce_sim']

        # ZZ: Add PE after prompting.
        # ToDo: Note there is a dropout in the output of PatchEmbedding's forward.
        enc_in = enc_in + self.position_embedding(enc_in)

        # ToDo: Update n_patches and patch_view_mask
        #  Multiply by n_channel if using channel-wise prompt
        n_patches += self.config.prompt_length * self.config.top_k

        # Passed input_mask must be None atm.
        input_mask = torch.ones(
            (batch_size, seq_len + self.config.prompt_length * self.config.top_k * self.patch_len)
        ).to(x_enc.device)

        patch_view_mask = Masking.convert_seq_to_patch_view(input_mask, self.patch_len)  # (bs, num_patches) all 1
        attention_mask = patch_view_mask.repeat_interleave(n_channels, dim=0)
        outputs = self.encoder(inputs_embeds=enc_in, attention_mask=attention_mask)
        enc_out = outputs.last_hidden_state
...
class L2pMOMENTPipeline(L2pMOMENT, PyTorchModelHubMixin):
    def __init__(self, config: Namespace | dict, **kwargs: dict):
        self._validate_model_kwargs(**kwargs)
        self.new_task_name = kwargs.get("model_kwargs", {}).pop(
            "task_name", TASKS.RECONSTRUCTION
